...
for ( $day=$first; strtotime($day)<=strtotime($now); $day=date("Y-m-d",strtotime("+1 Day",strtotime(date($day)))) ) {
	...
	$charts['temperature']['dates'][] = date("m/d/Y (D)",strtotime($day));
	...
	$charts['humidity']['dates'][] = date("m/d/Y (D)",strtotime($day));	
	$humidity = 0;
	if (count($q_humidity)) {
	...
	$charts['soil']['dates'][] = date("m/d/Y (D)",strtotime($day));	
	$soil = 0;
	if (count($q_soil)) {
	...
	$charts['dew']['dates'][] = date("m/d/Y (D)",strtotime($day));	
	$moisture_dew = 0;
	...
	$charts['rain']['dates'][] = date("m/d/Y (D)",strtotime($day));	
	$moisture_rain = 0;
